...
class Map {
	...
	/**
	 * Массив изображений карты. Первая размерность - Х:<br>
	 * imgs[x][y]
	 */
	Image[][] imgs;
	/** Изображение-заглушка. Отображается перед загрузкой карты */
	Image nullimg;
	int[] imgsx;
	int[] imgsy;
	...
	public Map(panel p) {
		System.out.println("Начинаем загрузку карты");
		panel = p;
		int max_x=0, max_y=0;
		// Загрузка размера карты из количества изображений
		// Благодаря этому мы можем "скармливать" программе карту любых размеров (предварительно её порезав Кромсатором)
		String path = new File("").getAbsolutePath();
		File dir = new File(path+ "/src/img/maps/"); //path указывает на директорию
		...
		// max_x и max_y не надо делать -1, потому что нумерация их начинается с нуля
		mapW = (max_x ) * pieceW + z.getWidth(null);
		mapH = (max_y ) * pieceH + z.getHeight(null);
		System.out.println("Размер карты: "+mapW+", "+mapH);
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Ошибка загрузки изображения подложки");
		}
		...
		// Это сильно тормозит загрузку игры
		// TODO: 1) иконки городов должны быть маленькие (например, 60х60 px)
		// TODO: 2) Карту города НЕ грузить (грузить только в том случае, если
		// персонаж при загрузке игры находится в городе)
		/*
		...
		System.out.println("Завершили загрузку карты");
	...
	public void draw(Graphics g) {
		// Это сделано, чтобы убрать белые полосы
		...
		/*
		 * // "загрузка" g.drawRect((panel.getWidth() - 300) / 2, 30, 300, 30);
		 ...
		 * g.drawString("Загрузка изображения", (panel.getWidth() - 300) / 2 +
		 * 30, 55); }
		 */
	}

	/**
	 * Определяет, видимо ли изображение с номерами x и y
	 * 
	 * @param x_img
	 *            - порядковый номер изображения по X ( НЕ его координата)
	 * @param y_img
	 *            - порядковый номер изображения по Y ( НЕ его координата)
	 * @return истина, если изображение с заданными номерами видимо на экране
	 */
	...
	/**
	 * Загружает изображение из ресурсов и записывает его в переданную
	 * переменную Image. Изображение будет искаться в папке img/ <br>
	 * Имя файла должно быть mapX,Y.png<br>
	 * Например при X = 2, Y = 1 файл должен иметь имя map2,1.png
	 * 
	 * @param filename
	 *            - имя файла изображения для загрузки
	 * @return Возвращает загруженное изображение Image
	 * @see Map#draw(Graphics)
	 */
	private Image loadImage(String filename) {
		// Загрузка изображения должна быть в потоке
		// Функция не должна создавать новый поток,
		// если уже был создан поток для
		// загрузки изображения из этого файла
	...
	private class MyThread extends Thread {
		...
		public void run() {
			// если скроллить во время загрузки изображения - будет ошибка
			// Добавление в этот ArrayList запрещает скроллить
			 ...
			 * e.printStackTrace(); }
			 */
			loadingImages.remove(res.getPath()); // иначе нельзя скроллить будет
		}
	}
	...
	public Point2D move(int mouseX, int mouseY) throws Exception {
		if (panel == null) {
			throw new Exception(
					"Карта не проициализирована панелью. И как её рисовать?");
			// return;
		}
		double OldX = mapX;
		double OldY = mapY;
		...
		if (mapY < 0)
			mapY = 0;
		// panel -то не уменьшается, уменьшается только размер изобр и X
