...
public class Redactor implements MouseMotionListener, MouseListener, Drawable {
    ...
    public Redactor() {
        ...
        for (int i = 0; i < nameLocation.length; i++) {
            ...
            bg.add(radBtn[i]);
           // game.Okno.p.add(radBtn[i]);

        }
        radBtn[0].setSelected(true);
        //game.Okno.p.add(btnNewButton_1);
        //game.Okno.p.add(btnNewButton_2);
        //game.Okno.p.addMouseListener(this);
        //game.Okno.p.addMouseMotionListener(this);
    ...
    protected void writeJson() {
        ...
        if (locations.size() > 0)
            for (int i = 0; i < locations.size(); i++) {//переносим все точки полигона в масивы в специальный класс т.к. Gson(библиотека) не умеет хранить Poligon
                //при добавлении новой перемнной в класс Redactor.MapLocation нужно добавить в класс помошник и добавть ниже присвение перемнной из основного в помошник
    ...
    private void createLocationAddPointLocation(MouseEvent e) {
        ...
        if (e.getButton() == 1 && deletPoligon == false) {
            ...
            if ( true/*mapLocations.p.npoints >= 3 &&
