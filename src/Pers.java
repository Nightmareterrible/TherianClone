...
class Pers {
	...
	public static void move(float mx1, float my1) throws Exception {
		...
		if (panel == null) {
			throw new Exception(
					"Pers не проициализирован панелью. И как его рисовать?");
			// return;
		}
		StartX = X;
		StartY = Y;
		...
		t1 = new Timer(50, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// обязательно DestX, Y - потому что они могут подвинуться
				// вместе с картой
	...
	public static void draw(Graphics g) {
		...
		if (X + PersR < 0 || Y + PersR < 0 || r(X) > panel.getWidth()
				|| r(Y) > panel.getHeight()) {
			// Находим точку пересечения с осями X и Y. Ту, которая не 0 -
			// подкрашиваем (рисуем там стрелочку)
			int interX = -1;
			int interY = -1;

			double x1 = panel.getWidth() / 2;
			double y1 = panel.getHeight() / 2; // центр координат
			double x2 = X;
			double y2 = Y;
			if (Math.abs(x2 - x1) < 1)// разница по модулю меньше 1 пикселя
										// (почти равны)
			{
				// вот тут косяк, надо что-то думать
				// хотя... точку пересечения прямой с границей экрана найти
				// легко:
				if (Y + PersR < 0)
					interY = 0;
				else
					interY = panel.getHeight();
				interX = (int) x1; // это и есть середина
			} else if (Math.abs(y2 - y1) < 1) // разница по модулю меньше 1
												// пикселя (почти равны)
			{
				// вот тут косяк, надо что-то думать
				// хотя... точку пересечения прямой с границей экрана найти
				// легко:
				interY = (int) y1; // это и есть середина
				if (X + PersR < 0)
					interX = 0;
				else
				...
				// x,y = 0,b - пересечение с OY
				// x,y = -b/k,0 - пересечение с OX
				double d_do_OX = dist(x1, y1, -b / k, 0);
				double d_do_OY = dist(x1, y1, 0, b);
				if (d_do_OX < d_do_OY) // !!!! не перепутай знак !!!
				{
			...
			g.fillOval(r(CircleX),  r(CircleY), PersR * 2, PersR * 2); // рисуем,
																				// что
																				// перс
																				// далеко
			// линия от центра координат к персонажу
			...
			if (img  != null)
				...
				g.fillOval(r(XX), r(YY), PersR * 2, PersR * 2); // рисуем персонажа
			CircleX = -100;
			CircleY = -100;
		}
